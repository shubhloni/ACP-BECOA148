...
public class SearchSupplier extends JFrame implements ActionListener
{
	...
	SearchSupplier()
	{
	    ...
	    l6.setBounds(500,60,380,50);l6.setForeground(Color.blue);
	    jf.add(l6);
		...
		t5.setBounds(670,280,200,25);
		jf.add(t5);


        		b0 = new JButton("Search",new ImageIcon("images//search.png"));
       		b0.setBounds(400,330,110,35);b0.setToolTipText("click to open supplier details");
		jf.add(b0);b0.addActionListener(this);

		b1 = new JButton("Clear",new ImageIcon("images//clear.png"));
		b1.setBounds(550,330,110,35);b1.setToolTipText("click to clear all textfields");
	   	jf.add(b1); b1.addActionListener(this);

      		b2= new JButton("All",new ImageIcon("images//all.png"));
		b2.setBounds(700,330,110,35);b2.setToolTipText("click to view all supplier details");
		jf.add(b2); b2.addActionListener(this);

		b3= new JButton("Back",new ImageIcon("images//all.png"));
		b3.setBounds(850,330,110,35);b3.setToolTipText("click to view all supplier details");
		jf.add(b3); b3.addActionListener(this);

      scrlPane.setBounds(0,380,1330,600);
      jf.add(scrlPane);
      tabGrid.setFont(new Font ("Times New Roman",0,15));
      tabGrid.setBackground(Color.pink);
	  ...
	  try
	   {
	   	if((( t1.getText()).equals("")) && (( t2.getText()).equals("")))
	        {
		    JOptionPane.showMessageDialog(this,"Please enter supplier name or supplier id !");//,"Warning!!!",JOptionPane.WARNING_MESSAGE);
	        }
	        else
	        {
	         int foundrec = 0;
		...
		System.out.println("Connected to database.");
         		ps=con.prepareStatement("select * from DDDD where sid='"+t1.getText()+"' or sname='"+t2.getText()+"'");   
	      	rs=ps.executeQuery();
		  while(rs.next())
	      {
    ...
    {//list
     ...
     try
     {
        ...
        rs = stmt.executeQuery("SELECT * from DDDD" );
          while(rs.next())
            {
            	model.insertRow(r++, new Object[]{rs.getString(2),rs.getString(3),rs.getString(4),rs.getString(5) });
            }
             con.close();
       }
      catch(SQLException se)
