    ...
    l11.setBounds(470,260,250,25);
        ...
        try
		{
			...
			System.out.println("Connected to database.");
			 ps=con.prepareStatement("select sname from DDDD");
		    rs=ps.executeQuery();
    		while(rs.next())
    		{
	 ...
	 if(ae.getSource()==b0)
		{
	       	 ...
	       	 else
		   {
			...
			ps=con.prepareStatement("Select sid from DDDD where sname='"+s+"'");
 			...
 			ps=con.prepareStatement("insert into MyDatabase (mb,mname,mcompany,mqty,mexpdate,mpurdate,mtype,mpurprice,msaleprice,mrackno,sid,sname)values(?,?,?,?,?,?,?,?,?,?,?,?)");
